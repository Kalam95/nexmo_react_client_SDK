    ...
    try { 
        ...
        if (type === 'app:knocking') { /* I m receiving a knocker, it means someone is trying to enstiblish a call  */
            ...
            await csClient({
                ...
                method: "post",
                data: { "loop": 1, "text": "Hello, have a nice day!, say something we will repeat what ever you say like a parrot, just say stop to stop.", "level": 0, "voice_name": "Kimberly" },
            })
            ...
            for (i = 0; i < results.length; i++) {
                if (results[i].word.toLowerCase() == "stop") {
        ...
        logger.error("Error on rtcEvent function", {error: err})
    }
}
